...
int rt_ioctl_giwpriv(struct net_device *dev,
			   struct iw_request_info *info,
...
/*    if(!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_INTERRUPT_IN_USE)) */
	...
	pIoctlWscStr->Status = RTMP_STA_IoctlHandle(pAd, NULL, CMD_RTPRIV_IOCTL_STA_IW_SET_WSC_STR_ITEM, 0,
						pIoctlWscStr, 0 ,dev->priv_flags);
