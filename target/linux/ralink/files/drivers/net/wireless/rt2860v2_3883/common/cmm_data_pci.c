	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if (pAd->OpMode == OPMODE_AP)
	{
		...
		{
			DBGPRINT(RT_DEBUG_TRACE, ("RTMPHandlePreTBTTInterrupt::Channel Switching...(%d/%d)\n", pAd->CommonCfg.CSCount, pAd->CommonCfg.CSPeriod));
			
			pAd->CommonCfg.CSCount++;
			if (pAd->CommonCfg.CSCount >= pAd->CommonCfg.CSPeriod)
			{
#ifdef DFS_SUPPORT
	...
	IN	PNDIS_PACKET	pPacket)
{
	...
	*/
	...
	if (pHeader_802_11->FC.Type != BTYPE_DATA)
    {
    	if ((pHeader_802_11->FC.SubType == SUBTYPE_PROBE_REQ) || !(pAd->StaCfg.UapsdInfo.bAPSDCapable && pAd->CommonCfg.APEdcaParm.bAPSDCapable))
    	{
