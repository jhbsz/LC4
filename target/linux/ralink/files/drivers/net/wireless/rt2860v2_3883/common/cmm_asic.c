	...
	IN PCHAR 					pDeltaPowerByBbpR1)
{
	...
	if (pAd->Mlme.OneSecPeriodicRound % 4 == 0)
	{
		...
		if (bAutoTxAgc)
		{
			...
			/* TSSI representation */
			if (IS_RT2860(pAd) || IS_RT2872(pAd)) /* 8-bits */
			{
				BbpR49.byte = (BbpR49.byte & 0xFF);
			}
			else if (IS_RT3071(pAd) || IS_RT3390(pAd) || IS_RT3090A(pAd) || IS_RT3572(pAd)) /* 5-bits */
			{
				BbpR49.byte = (BbpR49.byte & 0x1F);
			}
			else
			{
				DBGPRINT(RT_DEBUG_ERROR, ("%s: Unknow IC = 0x%X\n", __FUNCTION__, pAd->MACVersion));
			}
	...
	IN PCHAR 				pDeltaPowerByBbpR1)
{
	...
	*pTotalDeltaPwr = 0;
	...
	{ 
		...
		DBGPRINT(RT_DEBUG_INFO, ("[temp. compensation] idxTxPowerTable %d, idxTxPowerTable2 %d\n",
		...
		TxPowerTuningTableEntry0 = &TxPowerTuningTable[pAd->TxPowerCtrl.idxTxPowerTable 
								+ pAd->TxPowerCtrl.LookupTableIndex 
...
#endif /* DOT11_N_SUPPORT */
		TxPowerTuningTableEntry1 = &TxPowerTuningTable[pAd->TxPowerCtrl.idxTxPowerTable2 
								+ pAd->TxPowerCtrl.LookupTableIndex 
								+ TX_POWER_TUNING_ENTRY_OFFSET
#ifdef DOT11_N_SUPPORT				
		...
		DBGPRINT(RT_DEBUG_INFO, ("[temp. compensation] (tx0)RF_TX_ALC = %x, MAC_PowerDelta = %d\n",
			TxPowerTuningTableEntry0->RF_TX_ALC, TxPowerTuningTableEntry0->MAC_PowerDelta));
		DBGPRINT(RT_DEBUG_INFO, ("[temp. compensation] (tx1)RF_TX_ALC = %x, MAC_PowerDelta = %d\n",
			TxPowerTuningTableEntry1->RF_TX_ALC, TxPowerTuningTableEntry1->MAC_PowerDelta));
	...
	IN PRTMP_ADAPTER pAd) 
{
	...
	RTMP_CHIP_ASIC_AUTO_AGC_OFFSET_GET(
		pAd, &DeltaPwr, &TotalDeltaPower, &TxAgcCompensate, &BbpR49, &DeltaPowerByBbpR1);

	DBGPRINT(RT_DEBUG_INFO, ("%s: DeltaPwr=%d, TotalDeltaPower=%d, TxAgcCompensate=%d, BbpR49=0x%x, DeltaPowerByBbpR1=%d\n",
			__FUNCTION__,
			DeltaPwr,
			TotalDeltaPower,
			TxAgcCompensate,
