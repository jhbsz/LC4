...
INT Set_AsicCheck_Proc(
	...
	IN PSTRING arg)
{
	pAd->BBPResetCtl.AsicCheckEn = (BOOLEAN) simple_strtol(arg, 0 ,10);

	if (pAd->BBPResetCtl.AsicCheckEn)
		DBGPRINT(RT_DEBUG_OFF, ("turn on asic check feature\n"));
	else
		DBGPRINT(RT_DEBUG_OFF, ("turn off asic check feature\n"));
...
/*			kfree(pKey); */
		...
		case OID_802_11_P2P_MODE:
			...
			{
				/* Support p2p device enable Auto GO */
				/*UCHAR Mode=1;*/
				/*Status = copy_from_user(&Mode, wrq->u.data.pointer, wrq->u.data.length); */
...
*/
		...
		case OID_P2P_WSC_PIN_CODE:	
			DBGPRINT(RT_DEBUG_ERROR, ("Set::OID_P2P_WSC_PIN_CODE \n"));
            if (wrq->u.data.length != 8) /* PIN Code Length is 8 */
                Status = -EINVAL;
            else
            {
    ...
    IN  INT                 cmd)
{
    ...
    switch(cmd) 
    {
        ...
        case RT_OID_VERSION_INFO:
			...
			wrq->u.data.length = strlen(STA_DRIVER_VERSION) + 1;
			memset(driverVersion, 0 , 20);
...
/*                kfree(pStaConfig); */
		...
		case RT_OID_WSC_QUERY_STATUS:
			...
			DBGPRINT(RT_DEBUG_INFO, ("Query::RT_OID_WSC_QUERY_STATUS (=%d)\n", pAd->StaCfg.WscControl.WscStatus));
			break;
...
/*					kfree(pChListBuf); */
		...
		case OID_802_11_P2P_MODE:			
			...
			DBGPRINT(RT_DEBUG_TRACE, ("Query::OID_802_11_P2P_MODE (Len=%d, Rule=%c)\n", sizeof(char),pAd->P2pCfg.Rule));
			break;
		...
		case OID_802_11_P2P_SCAN_LIST:			
			...
			if (pUI_table)
			{
				...
				DBGPRINT(RT_DEBUG_TRACE, ("Query::OID_802_11_P2P_SCAN_LIST (Len=%d, GroupOpChannel=%d)\n", sizeof(char),pAd->P2pCfg.GroupOpChannel));
