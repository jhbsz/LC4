	...
	OUT STATE_MACHINE_FUNC Trans[])
{
    ...
    for (k = 0; k < MAX_APCLI_NUM; k++)
        pAd->ApCfg.ApCliTab[k].FailConntCounter; //Linda add for counting that connect to other AP failure time.
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	USHORT ifIndex = (USHORT)(Elem->Priv);
	PULONG pCurrState = &pAd->ApCfg.ApCliTab[ifIndex].CtrlCurrState;
    UINT  *failureConntCnter = &pAd->ApCfg.ApCliTab[ifIndex].FailConntCounter;      //Linda add for counting that connect to other AP failure time.
    ...
    (*failureConntCnter)++; 
    ApCliCtrlCntFailPrint(*failureConntCnter, "Use wrong security protocol or encryption method to connect to access-point");
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	USHORT ifIndex = (USHORT)(Elem->Priv);
	PULONG pCurrState = &pAd->ApCfg.ApCliTab[ifIndex].CtrlCurrState;
    UINT  *failureConntCnter = &pAd->ApCfg.ApCliTab[ifIndex].FailConntCounter;  //Pan add for counting that connect to other AP failure time.
