	...
	*(pDest + 2) = 0; 		/* Set length to 0 first.  Need to update to real length in the end of this function. */
	...
	for (i = 0;i < MAX_P2P_GROUP_SIZE;i++)
	{
		...
		if (pClient->P2pClientState == P2PSTATE_CLIENT_WPS_DONE)
		{
			...
			RTMPMoveMemory(ClientInfo.ConfigMethod, &pClient->ConfigMethod, 2);
		...
		*pDest = SubId;
		...
		switch(SubId)
		{
			...
			case SUBID_P2P_DEVICE_INFO: /* 13 */
				*pDest = 17 + pP2PCtrl->DeviceNameLen + 4;
				*(pDest + 1) = 0;
				RTMPMoveMemory(pDest + 2, pInBuffer, MAC_ADDR_LEN);
				/* DEvice Type */
				*((PUSHORT) (pDest + 8)) = cpu2be16(pP2PCtrl->ConfigMethod);
				/*RTMPMoveMemory(pDest + 10, &pAd->StaCfg.WscControl.RegData.SelfInfo.PriDeviceType[0], 8); */
				if (P2P_GO_ON(pAd))
				...
				*((PUSHORT) (pDest + 2)) = cpu2be16(pP2PCtrl->DeviceNameLen);
				...
				Length = 20 + pP2PCtrl->DeviceNameLen + 4;
				break;
			case SUBID_P2P_GROUP_INFO:	/* 14 */
				break;
			case SUBID_P2P_GROUP_ID:	/* 15 */
				*pDest = 6 + pAd->ApCfg.MBSSID[MAIN_MBSSID].SsidLen;
				*(pDest + 1) = 0;
				RTMPMoveMemory(pDest + 2, pInBuffer, MAC_ADDR_LEN);
				/*RTMPMoveMemory(pDest + 8, &pP2PCtrl->SSID[0], pP2PCtrl->SSIDLen); */
				RTMPMoveMemory(pDest + 8, &pAd->ApCfg.MBSSID[MAIN_MBSSID].Ssid, pAd->ApCfg.MBSSID[MAIN_MBSSID].SsidLen);
				
				/*Length = 9 + pP2PCtrl->SSIDLen; */
				Length = 9 + pAd->ApCfg.MBSSID[MAIN_MBSSID].SsidLen;
		...
		if (P2P_TEST_FLAG(pP2pEntry, P2PFLAG_PROVISIONED))
		{
			...
			switch(pP2pEntry->ConfigMethod)
			{
				...
				case WSC_CONFMET_LABEL:
					if ((Dpid != DEV_PASS_ID_REG) && (Dpid != DEV_PASS_ID_PIN))
					{
						DBGPRINT(RT_DEBUG_ERROR, (" P2P -2 Ignore Go Negociation Response with wrong dpid \n"));
						return;
					}
					break;
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
		...
		Intent = TempIntent >>1;
		DBGPRINT(RT_DEBUG_ERROR, ("    DevAddr in P2P IE = %02x %02x %02x %02x %02x %02x  \n",  DevAddr[0], DevAddr[1], DevAddr[2], DevAddr[3], DevAddr[4], DevAddr[5]));
		DBGPRINT(RT_DEBUG_ERROR, ("    Addr2 = %02x %02x %02x %02x %02x %02x  \n",  pFrame->p80211Header.Addr2[0], pFrame->p80211Header.Addr2[1],pFrame->p80211Header.Addr2[2],pFrame->p80211Header.Addr2[3],pFrame->p80211Header.Addr2[4],pFrame->p80211Header.Addr2[5]));
		DBGPRINT(RT_DEBUG_ERROR, ("    its bssid = %02x %02x %02x %02x %02x %02x  \n",  BssidAddr[0], BssidAddr[1],BssidAddr[2],BssidAddr[3],BssidAddr[4],BssidAddr[5]));
		DBGPRINT(RT_DEBUG_ERROR, ("    Dpid = %x ,%s \n", Dpid, decodeDpid(Dpid)));
	
		index = P2pGroupTabSearch(pAd, DevAddr);
		 
		DBGPRINT(RT_DEBUG_ERROR, ("P2P -Peer[%d] Intent = %x . ListenChannel = %x. OpChannel = %x. My Intent = %x. TempIntent = %x\n", 
									index, Intent, Channel, OpChannel, pP2PCtrl->GoIntentIdx, TempIntent));
		if (index == P2P_NOT_FOUND)
		{
			DBGPRINT(RT_DEBUG_ERROR, ("P2P -1  insert\n"));
			...
			RspStatus = P2PSTATUS_INFO_UNKNOWN;
		}
		...
		if (index < MAX_P2P_GROUP_SIZE)
		{
			pP2pEntry = &pAd->P2pTable.Client[index];
			pP2pEntry->ConfigMethod = ConfigMethod;
			DBGPRINT(RT_DEBUG_ERROR, ("My P2P Dpid = %x.    State = %s.		index = %d\n", 
										pP2PCtrl->Dpid, decodeP2PClientState(pP2pEntry->P2pClientState), index));
			if (IS_P2P_PEER_CLIENT_OP(pP2pEntry) || IS_P2P_PEER_WPAPSK(pP2pEntry) || IS_P2P_PEER_PROVISIONING(pP2pEntry))
			{
				...
				DBGPRINT(RT_DEBUG_ERROR, (" P2P - Existing peer  state is %d. %s,.	Delete it.\n", pP2pEntry->P2pClientState, decodeP2PClientState(pP2pEntry->P2pClientState)));
	
				return;
			}
			if (pP2pEntry->P2pClientState > P2PSTATE_GO_DONE)
			{
				DBGPRINT(RT_DEBUG_ERROR, (" P2P - Existing peer  state is %d . %s,. return.\n", pP2pEntry->P2pClientState, decodeP2PClientState(pP2pEntry->P2pClientState)));
				return;
			}
			else if (pP2pEntry->P2pClientState == P2PSTATE_SENT_GO_NEG_REQ)
			{
				...
				{
					DBGPRINT(RT_DEBUG_ERROR, (" P2P - Dual GO Req. Existing peer  state is %s. \n", decodeP2PClientState(pP2pEntry->P2pClientState)));
					return;
				}
			}
			if (P2P_GO_ON(pAd))
		...
		DBGPRINT(RT_DEBUG_ERROR, (" P2P - ReceDpid = %x. SentDpid %x \n", (Dpid), (SentDpid)));
		DBGPRINT(RT_DEBUG_ERROR, (" P2P - ReceDpid = %s. SentDpid %s \n", decodeDpid(Dpid), decodeDpid(SentDpid)));
		...
		DBGPRINT(RT_DEBUG_ERROR, (" P2P - Make Go Negociation Responce Length = %ld.RspStatus = %d \n", FrameLen, RspStatus));
		...
		if (RspStatus == P2PSTATUS_PASSED)
		{
			...
			P2pCmd.Idx = index;
			/*printk("<<<<<<<<<<<<<<<<< Mlme Enqueue, State = %d.    %s. !!!!\n", pP2PCtrl->GoFormCurrentState, decodeGroupFormationState(pP2PCtrl->GoFormCurrentState)); */
...
/*	MLME_P2P_ACTION_STRUCT	P2PActReq; */	
	...
	if (((pP2PCtrl->bSigmaEnabled == TRUE) && (DevDiscCnt == 0))||
		((pMacEntry != NULL) && (pEntry != NULL)
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	...
	if (bReinvoke == TRUE)
	{
		...
		P2pCheckInviteReq(pAd, (pAd->P2pTable.PerstEntry[index].MyRule == P2P_IS_GO), index, ChannelList, DevAddr, OpChannel, Ssid, SsidLen, &RspStatus);
		if (RspStatus == P2PSTATUS_SUCCESS)
		{
	...
	/* Start Provision */
	if ((bReinvoke == FALSE) && (MyRule == P2P_IS_CLIENT) && (RspStatus == P2PSTATUS_SUCCESS))
	{
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	PRT_P2P_CONFIG	pP2PCtrl = &pAd->P2pCfg;
	PP2P_PUBLIC_FRAME	pFrame = (PP2P_PUBLIC_FRAME)Elem->Msg;
	UCHAR		DevType[8], devAddr[6], BssidAddr[6], OpChannel, index, StatusCode;
	UCHAR		GroupCap, DeviceCap, DeviceNameLen;
	USHORT		ConfigMethod;
	...
	RTMPZeroMemory(ChannelList, 14);
	RTMPZeroMemory(BssidAddr, 14);
	/* Get Request content capability */
	P2pParseSubElmt(pAd, &pFrame->ElementID, (Elem->MsgLen - LENGTH_802_11 - 8), 
		FALSE, NULL, &GroupCap, &DeviceCap, NULL, &DeviceNameLen, devAddr, NULL, 
	...
	if ((StatusCode == P2PSTATUS_SUCCESS) || (StatusCode == P2PSTATUS_PASSED))
	{
			...
			&& (IS_PERSISTENT_ON(pAd)))
		{
			...
			else
			{
				...
				index = P2pGroupTabInsert(pAd, pFrame->p80211Header.Addr2, P2PSTATE_REINVOKEINVITE_TILLCONFIGTIME, NULL, SsidLen, 0, 0);
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	...
	*/
	...
	if ((WpsConfigMethod == WSC_CONFMET_LABEL) 
		|| (WpsConfigMethod == WSC_CONFMET_DISPLAY)
		...
		|| (WpsConfigMethod == 0x18c))
	{
		...
		if (p2pindex < MAX_P2P_GROUP_SIZE)
		{
			...
			DBGPRINT(RT_DEBUG_ERROR, ("Update My Config Method to  %s \n", decodeConfigMethod(pP2PCtrl->ConfigMethod )));			
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	...
	DBGPRINT(RT_DEBUG_ERROR, ("P2pRece Provision RspAction ==>ConfigMethod = %x. %s.\n", ConfigMethod, decodeConfigMethod(ConfigMethod)));
	...
	if ((ConfigMethod == WSC_CONFMET_LABEL) 
		|| (ConfigMethod == WSC_CONFMET_DISPLAY)
...
/*	MLME_QUEUE_ELEM Elem;*/
	...
	if (Elem)
	{
		...
		P2PMakeProbe(pAd, Elem, pAd->MlmeAux.Channel, SUBTYPE_PROBE_REQ, pOutBuffer, &FrameLen);
		if (FrameLen > 0)
			MiniportMMRequest(pAd, 0, pOutBuffer, FrameLen);
		MlmeFreeMemory(pAd, pOutBuffer);
	}
	...
	IN PUCHAR		Addr1)
{
		...
		if (pP2pEntry->Rule == P2P_IS_CLIENT)
		{
			...
			TempLen = InsertP2PSubelmtTlv(pAd, SUBID_P2P_GROUP_ID, pP2PCtrl->CurrentAddress, pDest);
			FrameLen += TempLen;
			pDest += TempLen;
			P2pLen +=TempLen;
	...
	OUT PULONG		pTotalFrameLen)
{
	...
	*pTotalFrameLen += Length;
	...
	if (((TempIntent & 1) == 1) && (RealIntent == pP2PCtrl->GoIntentIdx))
	{
		RealIntent = TempIntent&0xfe;
		DBGPRINT(RT_DEBUG_TRACE, (" P2P -    1  %d \n", pP2PCtrl->GoIntentIdx));
	}
	else if (((TempIntent & 1) == 0) && (RealIntent == pP2PCtrl->GoIntentIdx))
	{
		RealIntent = TempIntent|0x1;
		DBGPRINT(RT_DEBUG_TRACE, (" P2P -    2  %d \n", pP2PCtrl->GoIntentIdx));
	}
	else
		RealIntent = pP2PCtrl->GoIntentIdx << 1;
	...
	*pTotalFrameLen += Length;
	/* attach subelementID= 6. */
		Length = InsertP2PSubelmtTlv(pAd, SUBID_P2P_LISTEN_CHANNEL, &Channel, pDest);
	pDest += Length;
 	P2pIeLen += Length;
	*pTotalFrameLen += Length;
	...
	*pTotalFrameLen += Length;
	...
	{
		Length = InsertP2PSubelmtTlv(pAd, SUBID_P2P_GROUP_ID, pP2PCtrl->CurrentAddress, pDest);
		pDest += Length;
		P2pIeLen += Length;
		*pTotalFrameLen += Length;
	}
	...
	OUT PULONG		pTotalFrameLen)
{
	...
	*pTotalFrameLen += Length;
	/*  attach subelementID= 6. */
		...
		Length = InsertP2PSubelmtTlv(pAd, SUBID_P2P_LISTEN_CHANNEL, &pP2PCtrl->GroupChannel, pDest);
	pDest += Length;
	P2pIeLen += Length;
	*pTotalFrameLen += Length;
		...
		switch(pEid->Eid)
		{
			...
			case IE_VENDOR_SPECIFIC:
				...
				if (NdisEqualMemory(pEid->Octet, P2POUIBYTE, 4) && (pEid->Len > 4))
				{
					...
					{
						RTMPMoveMemory(pP2pSubelement, pEid->Octet, pEid->Len);
	...
	IN PUCHAR		Bssid) 
{
	...
	DBGPRINT(RT_DEBUG_ERROR, ("P2pParseManageSubElmt  TotalNumOfP2pAttribute = %d.  \n", TotalNumOfP2pAttribute));
	/* If this AP carries Managed Attribute. Update to our ManageAPBSsid */
	if (P2pManageability != 0xff)
	{
		DBGPRINT(RT_DEBUG_ERROR, ("SYNC - Receive desired BEACON with valid P2pManageability= %x\n", P2pManageability));
		DBGPRINT(RT_DEBUG_ERROR, ("SYNC -NumOfOtherP2pAttri %d . Channel = %d. \n", NumOfOtherP2pAttri, Channel));
	...
	*/
									...
									P2pSubelement))
		{
			...
			if (index < MAX_P2P_GROUP_SIZE)
			{
				...
				pAd->P2pTable.Client[index].DevCapability = DeviceCap;
				pAd->P2pTable.Client[index].GroupCapability = GroupCap;

				/* Update Rule according to latest Probe Response or Beacon that I received. */
				if ((bBeacon == TRUE) && 
					((pAd->P2pTable.Client[index].P2pClientState == P2PSTATE_DISCOVERY_CLIENT) || (pAd->P2pTable.Client[index].P2pClientState == P2PSTATE_DISCOVERY)))
				{
						...
						bChangeState = TRUE;
						pAd->P2pTable.Client[index].Rule = P2P_IS_GO;
						pAd->P2pTable.Client[index].P2pClientState = P2PSTATE_DISCOVERY_GO;
						DBGPRINT(RT_DEBUG_ERROR, ("3 From DevAddr = %02x:%02x:%02x:%02x:%02x:%02x.	Change rule to GO GroupCap = %x \n",  PRINT_MAC(DevAddr), GroupCap));
				}
				else if ((pAd->P2pTable.Client[index].P2pClientState == P2PSTATE_DISCOVERY_GO)
					&& ((GroupCap&GRPCAP_OWNER) != GRPCAP_OWNER))
				{
					if ((pAd->P2pTable.ClientNumber < 10)
						|| (((pAd->P2pTable.ClientNumber % 4) == 3) && (pAd->P2pTable.ClientNumber >= 10)))
					bChangeState = TRUE;
					pAd->P2pTable.Client[index].Rule = P2P_IS_CLIENT;
					pAd->P2pTable.Client[index].P2pClientState = P2PSTATE_DISCOVERY;
				...
				if (!P2P_TEST_FLAG(&pAd->P2pTable.Client[index], P2PFLAG_PROVISIONED) && (ConfigMethod != 0xffff))
					pAd->P2pTable.Client[index].ConfigMethod = ConfigMethod;
				...
				{
					pAd->P2pTable.Client[index].Rule = P2P_IS_GO;
					RTMPMoveMemory(pAd->P2pTable.Client[index].Ssid, Ssid, 32);
					pAd->P2pTable.Client[index].SsidLen = SsidLen;
					RTMPMoveMemory(pAd->P2pTable.Client[index].bssid, Addr2, MAC_ADDR_LEN);
					RTMPMoveMemory(pAd->P2pTable.Client[index].InterfaceAddr, Addr2, MAC_ADDR_LEN);
					pAd->P2pTable.Client[index].OpChannel = Elem->Priv; /* WorkingChannel; */
				...
				else if (bBeacon == FALSE)
				{
					...
					else
					{
						...
						if (IS_P2P_PEER_DISCOVERY(&pAd->P2pTable.Client[index]))
							pAd->P2pTable.Client[index].P2pClientState = P2PSTATE_DISCOVERY;
						pAd->P2pTable.Client[index].Rule = P2P_IS_CLIENT;
					}
					/* P2P PrimaryDevType only appears in Probe Response. not in beacon. */
					RTMPMoveMemory(pAd->P2pTable.Client[index].PrimaryDevType, DevType, 8);
					/* Can I just set this channel that I got Probe response as listent channel?? */
					if (Channel == 0)
						pAd->P2pTable.Client[index].ListenChannel = pAd->LatchRfRegs.Channel;
					else
						pAd->P2pTable.Client[index].ListenChannel = Channel;
				...
				if ((pAd->P2pTable.Client[index].P2pClientState <= P2PSTATE_GO_DONE))
				{
					if ((DeviceCap & DEVCAP_INVITE) == DEVCAP_INVITE)
						P2P_SET_FLAG(&pAd->P2pTable.Client[index], P2PFLAG_INVITE_ENABLED);
					/*
					DBGPRINT(RT_DEBUG_TRACE, ("P2p : DevAddr[%02x:%02x:%02x:%02x:%02x:%02x] State = %s.\n", PRINT_MAC(DevAddr), decodeP2PClientState(pAd->P2pTable.Client[index].P2pClientState)));
					DBGPRINT(RT_DEBUG_TRACE, ("P2p : GrpCap=%x. DevCap=%x. ConfigMethod= %x.\n", GroupCap, DeviceCap, pAd->P2pTable.Client[index].ConfigMethod));
					DBGPRINT(RT_DEBUG_TRACE, ("P2p : DevAddr	 [%02x %02x %02x %02x %02x %02x]\n", PRINT_MAC(DevAddr)));
					 */

					if (pAd->P2pTable.Client[index].P2pClientState != P2PSTATE_DISCOVERY)
					{
						UCHAR ClientState = pAd->P2pTable.Client[index].P2pClientState;
						COPY_MAC_ADDR(&P2pCmd.Addr[0], pAd->P2pTable.Client[index].addr);
						P2pCmd.Idx = index;
					...
					if (pAd->P2pCfg.ConnectingIndex < MAX_P2P_GROUP_SIZE)
					{
						...
						{
							DBGPRINT(RT_DEBUG_INFO, ("    From : %02x:%02x:%02x:%02x:%02x:%02x, state = %s\n", PRINT_MAC(Addr2), decodeP2PClientState(pAd->P2pTable.Client[index].P2pClientState)));
							DBGPRINT(RT_DEBUG_INFO, ("    P2PTab[%d] Addr : %02x:%02x:%02x:%02x:%02x:%02x\n", index, PRINT_MAC(pAd->P2pTable.Client[index].addr)));
	...
	IN LARGE_INTEGER   TimeStamp) 
{
									...
									P2pSubelement))
	{
		...
		if (P2P_TEST_BIT(pAd->P2pCfg.CTWindows, P2P_OPPS_BIT))
		{
			...
			{
				DBGPRINT(RT_DEBUG_ERROR, ("%s::  set P2P CTWindows timer.\n", __FUNCTION__));
...
/*	IN BOOLEAN	bSendRsp)  */
{
									...
									WpsIE))
		{
			...
			if (bSendRsp == TRUE)
			{
				if (index < MAX_P2P_GROUP_SIZE)
				{
					DBGPRINT(RT_DEBUG_INFO, ("%s  \n", decodeP2PClientState(pAd->P2pTable.Client[index].P2pClientState)));
				}
				/* allocate and send out ProbeRsp frame */
				NStatus = MlmeAllocateMemory(pAd, &pOutBuffer);  /* Get an unused nonpaged memory */
				if (NStatus != NDIS_STATUS_SUCCESS)
				...
				P2PMakeProbe(pAd, Elem, Elem->Channel, SUBTYPE_PROBE_RSP, pOutBuffer, &FrameLen);
				if (FrameLen > 0)
					MiniportMMRequest(pAd, 0, pOutBuffer, FrameLen);
	...
	OUT	PULONG			pIeLen)
{
	...
	*((PUSHORT) (pData + 4)) = cpu2be16(pAd->P2pCfg.ConfigMethod);
...
/*		if ((pDevInfo->SelReg == 1) && (pAd->P2pCfg.Dpid == DEV_PASS_ID_PBC)) */
		...
		{
			if ((pAd->P2pCfg.P2PConnectState == P2P_ANY_IN_FORMATION_AS_GO) || IS_P2P_REGISTRA(pAd) || P2P_GO_ON(pAd) || (pAd->P2pCfg.GoIntentIdx == 16))
			{
...
/*				*(pData + 4) = pAd->StaCfg.WscControl.WscConfigMethods; */ /* STA */
	...
	{
		/* 5. DPID shall be a required attribute if Credentials are available and ready for immediate use. */
		if (pAd->P2pCfg.Dpid  != DEV_PASS_ID_NOSPEC)
		{
			/* Device Password ID */
			*((PUSHORT) pData) = cpu2be16(WSC_ID_DEVICE_PWD_ID);
	...
	OUT	PUCHAR			pIeLen)
{
	...
	*((PUSHORT) (pData + 2)) = cpu2be16(0x0008);	
	/*NdisMoveMemory((pData + 4), pDevInfo->PriDeviceType, 8); */
	if (P2P_GO_ON(pAd))
		NdisMoveMemory((pData + 4), AP_Wsc_Pri_Dev_Type, 8);
	else
		NdisMoveMemory((pData + 4), STA_Wsc_Pri_Dev_Type, 8);
	pData += 12;
	Len   += 12;
	...
	OUT	ULONG *pFrameLen) 
{
	...
	if (DsChannel == 0)
		DBGPRINT(RT_DEBUG_ERROR, ("SYNC - P2PMakeProbeRsp. DsChannel = is 0 !!!!!ComChannel = %d.\n",pAd->CommonCfg.Channel ));
	...
	IN UCHAR	*pRspStatus)
{
	...
	*pRspStatus = P2PSTATUS_SUCCESS;
	...
	if ((SsidLen > 0) && (!RTMPEqualMemory(pAd->P2pTable.PerstEntry[index].Profile.SSID.Ssid, Ssid, SsidLen)))
	{
		...
		DBGPRINT(RT_DEBUG_ERROR, ("Ssid2[%d] = %s.  \n",  pAd->P2pTable.PerstEntry[index].Profile.SSID.Ssid));
		*pRspStatus = P2PSTATUS_INVALID_PARM;
		return;
	}
