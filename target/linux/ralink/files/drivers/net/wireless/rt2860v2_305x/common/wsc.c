...
/*	if( (WscData = kmalloc(WSC_MAX_DATA_LEN, GFP_ATOMIC)) == NULL) */
	...
	switch (MsgType)
	{
		...
		case WSC_MSG_WSC_DONE:
			...
			{
				if (pEntry)
					WscDelListEntryByMAC(&pWscControl->WscPeerList,
										 pEntry->Addr);
#ifdef CONFIG_AP_SUPPORT
				if (CurOpMode == AP_MODE)
				{
...
{	
	...
	if (pWscControl->WscV2Info.bEnableWpsV2 &&
		(CurOpMode == STA_MODE))
		/*
			WSC 2.0 AP will disable WSC if auth mode is Shared or WPA(PSK),
			so STA's AuthTypeFlags only needs to include Open and WPA2PSK.
		*/
	...
	if (pWscControl->WscV2Info.bEnableWpsV2 &&
		(CurOpMode == STA_MODE))
		/*
			WSC 2.0 AP will disable WSC if cipher is WEP or TKIP,
			so STA's AuthTypeFlags only needs to include None and AES.
		*/
	...
	while (Length > 4)
	{
		...
		switch (ntohs(WscType))
		{
			...
			case WSC_ID_DEVICE_PWD_ID:
				NdisMoveMemory(&wsc_dev_pass_id, pData, sizeof(PUSHORT));
				wsc_dev_pass_id = be2cpu16(wsc_dev_pass_id);
				break;

			case WSC_ID_SEL_REG_CFG_METHODS:
				NdisMoveMemory(&wsc_sel_reg_conf_mthd, pData, sizeof(PUSHORT));
...
/*	UCHAR			OutMsgBuf[512];		// buffer to create message contents */
	...
	DBGPRINT(RT_DEBUG_TRACE, ("-----> WscBuildProbeReqIE\n"));
...
#ifdef P2P_SUPPORT	
	if (pAdapter->P2pCfg.bSigmaEnabled)		
		pWscControl->WscEnrolleePinCode = pWscControl->WscPinCode;
#endif /* P2P_SUPPORT */
