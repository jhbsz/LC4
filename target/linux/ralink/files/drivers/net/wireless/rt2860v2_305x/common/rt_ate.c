	...
	IN PRTMP_ADAPTER pAd)
{
	...
	*/
	...
	if (1)
	{
		...
		if (IS_RT5350(pAd))
		{
			RFValue2 = (FreqOffset & 0x7F); /* bit7 = 0 */
		}
		else
#endif /* RT5350 */
...
{    
	...
	if (!IS_RT5350(pAd) && !(pAd->TxPowerCtrl.bInternalTxALC))                
	{
...
{    
	...
	if ((!IS_RT3350(pAd)) && (!IS_RT3352(pAd)) && (!IS_RT3390(pAd)))                            
	{
		DBGPRINT(RT_DEBUG_TRACE, ("Not support TSSI calibration since not 3370/3390/3350/3352 chip !!!\n"));
		return FALSE;
	}
	else
	{                              
...
{    
	...
	if ((!IS_RT3350(pAd)) && (!IS_RT3352(pAd)) && (!IS_RT3390(pAd)))                                
	{
		DBGPRINT(RT_DEBUG_TRACE, ("Not support TSSI calibration since not 3370/3390/3350/3352 chip or EEPROM not set!!!\n"));
		return FALSE;
	}
	...
	if (IS_RT3352(pAd))
	{
		RT28xx_EEPROM_READ16(pAd, EEPROM_G_TX2_PWR_OFFSET+CurrentChannel-1, ChannelPower);
		pAd->ate.TxPower1 = (UCHAR)(ChannelPower & 0xff);
		DBGPRINT(RT_DEBUG_TRACE, ("Channel %d, Calibrated Tx.Power1= 0x%x\n", CurrentChannel, pAd->ate.TxPower1));
	}
