	...
	IN UCHAR ifIndex)
{
	...
	if (pApCliEntry->bP2pClient)
	{
		...
		if ((P2pIdx != P2P_NOT_FOUND) && (P2P_CLI_ON(pAd)) && 
			(IS_ENTRY_APCLI(pEntry)) && (pEntry->WpaState == AS_PTKINITDONE))
		{
			printk("%s::  Secured = %d.  Addr = %02x:%02x:%02x:%02x:%02x:%02x.\n", __FUNCTION__, pEntry->PortSecured, PRINT_MAC(pEntry->Addr));
		...
		switch (pEid->Eid)
		{
			...
			case IE_VENDOR_SPECIFIC:
				...
				if (NdisEqualMemory(pEid->Octet, P2POUIBYTE, sizeof(P2POUIBYTE)) && (pEid->Len >= 4))
				{
					...
					DBGPRINT(RT_DEBUG_ERROR, (" ! ===>P2P - %s	P2P IE Len becomes = %d.\n", __FUNCTION__, *P2PSubelementLen));
				}
				break;
				default:
