	...
	if ((pP2PCtrl->Rule == P2P_IS_GO) && (P2P_GO_ON(pAd)))
	{
		...
		if (pP2PCtrl->WscMode == WSC_PIN_MODE)
		{
			if (pP2PCtrl->ConfigMethod == WSC_CONFMET_KEYPAD)
			{
				...
				if (pP2PCtrl->bSigmaEnabled)
				{
				/* For Sigma Test Engine */
					DBGPRINT(RT_DEBUG_ERROR, ("%s:: I'm Registrar with KEYPAD!\n", __FUNCTION__));
				if (pP2PCtrl->ConfigMethod == WSC_CONFMET_KEYPAD) 
					Set_AP_WscPinCode_Proc(pAd, "12345670");
				}
				OS_WAIT(700);
			...
			{
			/* For Sigma Test Engine */
				DBGPRINT(RT_DEBUG_ERROR, ("%s:: I'm Registrar with DISPLAY!\n", __FUNCTION__));
			Set_AP_WscPinCode_Proc(pAd, "12345670");
			}
			OS_WAIT(700);
			Set_AP_WscGetConf_Proc(pAd, "1");
