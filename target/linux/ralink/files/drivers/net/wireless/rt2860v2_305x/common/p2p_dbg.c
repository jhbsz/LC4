	...
	IN	PSTRING 		arg)
{
	...
	if (P2P_INF_ON(pAd))
	{
		...
		DBGPRINT(RT_DEBUG_ERROR, ("p2pControl = %x.    Persistent = %s. Invite = %s.    ClientDiscovery = %s.    IntraBss = %s.    ExtListen = %s.\n", pP2PCtrl->P2pControl, (IS_PERSISTENT_ON(pAd))? "ON" : "OFF", (IS_INVITE_ON(pAd))? "ON" : "OFF",
	...
	IN	PSTRING			arg)
{
	...
	for (i=0; i < pAd->P2pTable.ClientNumber; i++)
	{
		...
		DBGPRINT(RT_DEBUG_ERROR, ("]\n"));
		/*
		DBGPRINT(RT_DEBUG_ERROR, ("]\n                         DevCapability = %x.\n", pAd->P2pTable.Client[i].DevCapability));
		decodeDeviceCap(pAd->P2pTable.Client[i].DevCapability);
		DBGPRINT(RT_DEBUG_ERROR, ("                                 ServiceDiscovery = %d.        P2P Client Discoverability = %d\n", (pAd->P2pTable.Client[i].DevCapability & 0x1), ((pAd->P2pTable.Client[i].DevCapability >> 1) & 0x1)));
		DBGPRINT(RT_DEBUG_ERROR, ("                                 ConcurrentOperation = %d.        InfraManaged = %d.\n", ((pAd->P2pTable.Client[i].DevCapability >> 2) & 0x1), ((pAd->P2pTable.Client[i].DevCapability >> 3) & 0x1)));
		DBGPRINT(RT_DEBUG_ERROR, ("                                 DeviceLimit = %d.        InvitationProcedure = %d.\n", ((pAd->P2pTable.Client[i].DevCapability >> 4) & 0x1), ((pAd->P2pTable.Client[i].DevCapability >> 5) & 0x1)));

		DBGPRINT(RT_DEBUG_ERROR, ("                         GroupCapability = %x.\n", pAd->P2pTable.Client[i].GroupCapability));
		decodeGroupCap(pAd->P2pTable.Client[i].GroupCapability);
		DBGPRINT(RT_DEBUG_ERROR, ("                                 GroupOwner = %d.        PersistentP2PGroup = %d\n", (pAd->P2pTable.Client[i].GroupCapability & 0x1), ((pAd->P2pTable.Client[i].GroupCapability >> 1) & 0x1)));
		DBGPRINT(RT_DEBUG_ERROR, ("                                 GroupLimit = %d.        IntraBSS = %d.\n", ((pAd->P2pTable.Client[i].GroupCapability >> 2) & 0x1), ((pAd->P2pTable.Client[i].GroupCapability >> 3) & 0x1)));
		DBGPRINT(RT_DEBUG_ERROR, ("                                 CrossConnection = %d.        PersistentReconnect = %d.\n", ((pAd->P2pTable.Client[i].GroupCapability >> 4) & 0x1), ((pAd->P2pTable.Client[i].GroupCapability >> 5) & 0x1)));
		DBGPRINT(RT_DEBUG_ERROR, ("                                 GroupFormation = %d.        \n", ((pAd->P2pTable.Client[i].GroupCapability >> 6) & 0x1)));
		 */
		...
		DBGPRINT(RT_DEBUG_ERROR, ("                         ChNumber = %d.        OpChannel = %d.        ListenChannel = %d.\n", pAd->P2pTable.Client[i].ChannelNr, pAd->P2pTable.Client[i].OpChannel, pAd->P2pTable.Client[i].ListenChannel));
		...
		DBGPRINT(RT_DEBUG_ERROR, ("                         P2pIP = %08x.        P2pFlag = %d.\n", (UINT32)pAd->P2pTable.Client[i].Peerip, pAd->P2pTable.Client[i].P2pFlag));
		DBGPRINT(RT_DEBUG_ERROR, ("                         Dpid = %d.        StateCount = %d.        Rule = %s.\n", pAd->P2pTable.Client[i].Dpid, pAd->P2pTable.Client[i].StateCount, decodeMyRule(pAd->P2pTable.Client[i].Rule)));
		DBGPRINT(RT_DEBUG_ERROR, ("                         GeneralToken = %d.        NoAToken = %d.\n", pAd->P2pTable.Client[i].GeneralToken, pAd->P2pTable.Client[i].NoAToken));
		DBGPRINT(RT_DEBUG_ERROR, ("                         RegClass = %d.        ConfigTimeOut = %d.\n", pAd->P2pTable.Client[i].RegClass, pAd->P2pTable.Client[i].ConfigTimeOut));
	...
	IN	PSTRING			arg)
{
	...
	DBGPRINT(RT_DEBUG_ERROR, ("p2p_OpStatus = %d\n", pAd->flg_p2p_OpStatusFlags));
