	...
	IN	RTMP_ADAPTER *pAd)
{
	...
	/* default RT3050 */
	EeBuffer = RT3050_EeBuffer;

#ifdef RT3052
	if (IS_RT3052(pAd))
		EeBuffer = RT3052_EeBuffer;
#endif // RT3052 //
}
	...
	IN PRTMP_ADAPTER		pAd)
{
	...
	pChipOps->VdrTuning1 = RT305x_VdrTuning1;

	pChipOps->ChipSpecific = RT305x_ChipSpecific;
}
	...
	IN PRTMP_ADAPTER		pAd)
{
	if ((pAd->CommonCfg.CN >> 16) == 0x3333)
	{
		...
		// For 1x1 designs, R105=0x31. But for 2x2 (3352, etc), then we should set R105=0x35, to enable MLD for 2SS Rx
		//
...
/* --------------------------- local functions ------------------------------ */
static VOID RT305x_PowerHighPatchAP(
	IN PRTMP_ADAPTER 			pAd)
{
...
static VOID RT305x_PowerHighPatchSTA(
	IN PRTMP_ADAPTER 			pAd)
{
	...
	IN ULONG					Data)
{
	switch(StateId)
	{
		case RTMP_CHIP_SPEC_STATE_WMODE_CMD:
			switch(FuncId)
			{
				case RTMP_CHIP_SPEC_WLAN_MODE_CHANGE:
					RT305x_WlanModeChange(pAd);
					break;
			}
			break;

		case RTMP_CHIP_SPEC_STATE_INIT:
			switch(FuncId)
			{
				case RTMP_CHIP_SPEC_INITIALIZATION:
					RT305x_SpecificInit(pAd);
					break;
			}
			break;

		case RTMP_CHIP_SEPC_STATE_HT_SET:
			switch(FuncId)
			{
				case RTMP_CHIP_SPEC_HT_MODE_CHANGE:
					RT305x_HtModeChange(pAd, Data);
					break;
			}
			break;

		case RTMP_CHIP_SPEC_STATE_AP_PERIODIC:
			switch(FuncId)
			{
				case RTMP_CHIP_SPEC_HIGH_POWER_PATCH_AP:
					RT305x_PowerHighPatchAP(pAd);
					break;
			}
			break;

		case RTMP_CHIP_SPEC_STATE_STA_PERIODIC:
			switch(FuncId)
			{
				case RTMP_CHIP_SPEC_HIGH_POWER_PATCH_STA:
					RT305x_PowerHighPatchSTA(pAd);
					break;
			}
			break;
	}
