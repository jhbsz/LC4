...
static int power_button_prep_sysfs(void)
{
	...
	power_button_driver.kset.kobj.ktype = &ktype_power_button;

	err = kset_register(&power_button_driver.kset);
	if (err)
		return err;
	...
	kobject_init(&power_button_driver.power_button,power_button_driver.kset.kobj.ktype);
...
static int power_button_build_sysfs(void) {
	...
	return kobject_add(&power_button_driver.power_button,NULL,"power-button-1");
}
