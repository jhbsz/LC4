...
unsigned long ox810sata_command_active = 0;
EXPORT_SYMBOL(ox810sata_command_active);
...
static unsigned int ox810sata_qc_issue(struct ata_queued_cmd *qc)
{
    ...
    u32 raid_reg = OXNASSATA_NOTRAID; /* default to no raid */
    int port0fail = 0;
    int port1fail = 0;
    ...
    if (unlikely(this_port_fail)) {
        /* record the error */
        qc->err_mask |= AC_ERR_ATA_BUS;

        /* call ata qc complete */
        ...
        case OXNASSATA_RAID0:
            ...
            /* Set RAID controller to do RAID-0 */
            writel(OX810SATA_RAID_ONE_SECTOR_STRIPE, OX810SATA_STRIPE_SIZE);
            writel(OX810SATA_RAID0, OX810SATA_RAID_CONTROL);
            writel(OX810SATA_RAID_TWODISKS, OX810SATA_RAID_SET);
            break;
        case OXNASSATA_RAID1:
            /* set interrupt mode for raid controller interrupts only */
            writel(OX810SATA_RAID_INTS_WANTED, OX810SATA_CORE_IES );  

            /* Set RAID controller to do RAID-1 */
            writel(OX810SATA_RAID1, OX810SATA_RAID_CONTROL);
            writel(OX810SATA_RAID_TWODISKS, OX810SATA_RAID_SET);
            break;
        default:
