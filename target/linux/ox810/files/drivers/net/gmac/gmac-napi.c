...
#define COPRO_TX_QUEUE_NUM_ENTRIES  8
...
#define ALL_RX_PACKETS_IN_PAGES
#endif
...
static void refill_rx_ring(struct net_device *dev)
{
	...
	if (unlikely(priv->rx_buffers_per_page_)) {
		// Receive into pages
		struct page *page = 0;
		int offset = 0;
...
static int gmac_link_state_init_sysfs(gmac_priv_t* priv)
{
	...
	priv->link_state_kset.kobj.ktype = &ktype_gmac_link_state;
	...
	kobject_init(&priv->link_state,priv->link_state_kset.kobj.ktype);
	...
	return kobject_add(&priv->link_state_kobject,NULL,&priv->link_state_kset.kobj);
}
...
not_valid_skb:
	...
	if (is_rx_collision_error(desc_status)) {
		DBG(20, KERN_INFO "process_rx_packet() %s: Collision (status: 0x%08x)\n", priv->netdev->name, desc_status);
		++priv->stats.collisions;
	}
	if (is_rx_crc_error(desc_status)) {
		DBG(20, KERN_INFO "process_rx_packet() %s: CRC error (status: 0x%08x)\n", priv->netdev->name, desc_status);
		++priv->stats.rx_crc_errors;
		++priv->stats.rx_errors;
	}
	if (is_rx_frame_error(desc_status)) {
		DBG(20, KERN_INFO "process_rx_packet() %s: frame error (status: 0x%08x)\n", priv->netdev->name, desc_status);
		++priv->stats.rx_frame_errors;
		++priv->stats.rx_errors;
	}
	if (is_rx_length_error(desc_status)) {
		DBG(20, KERN_INFO "process_rx_packet() %s: Length error (status: 0x%08x)\n", priv->netdev->name, desc_status);
		++priv->stats.rx_length_errors;
		++priv->stats.rx_errors;
	}
	if (is_rx_csum_error(desc_status)) {
		DBG(20, KERN_INFO "process_rx_packet() %s: Checksum error (status: 0x%08x)\n", priv->netdev->name, desc_status);
		++priv->stats.rx_frame_errors;
		++priv->stats.rx_errors;
	}
	...
	while (1) {
		...
		if (first) {
			...
			if (desc_len > hlen) {
...
#ifdef CONFIG_OXNAS_ZERO_COPY_RX_SUPPORT
				// Need all of packet in pages regardless of whether we've also
				// copied it into the skb. Size of zero should stop stack from
				// using any data from this page, but the ZeroCopyRx code can
				// still infer the length from skb->len
				skb_frag_t *frag = &skb_shinfo(skb)->frags[0];
				...
				frag->size		  = 0;
#else // CONFIG_OXNAS_ZERO_COPY_RX_SUPPORT
...
{
    gmac_priv_t *priv = (gmac_priv_t*)netdev_priv(dev);
#ifndef ALL_RX_PACKETS_IN_PAGES
...
#endif // ALL_RX_PACKETS_IN_PAGES
#ifndef CONFIG_OXNAS_RX_BUFFER_SIZE
		priv->rx_buffer_size_ = max_packet_buffer_size;
#else // !CONFIG_OXNAS_RX_BUFFER_SIZE
		priv->rx_buffer_size_ = CONFIG_OXNAS_RX_BUFFER_SIZE;
#endif // ! CONFIG_OXNAS_RX_BUFFER_SIZE
    ...
    struct net_device *dev)
{
    ...
    if (skb_shinfo(skb)->frag_list) {
		...
		printk("Linearizing skb with frag_list\n");
		err = skb_linearize(skb);
        if (err) {
...
#elif !defined(CONFIG_LEON_COPRO)
    // Do TX H/W checksum and SG list processing
    netdev->features |= NETIF_F_HW_CSUM;
    netdev->features |= NETIF_F_SG;
#endif // USE_TX_CSUM
