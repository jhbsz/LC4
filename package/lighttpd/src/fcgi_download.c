    ...
    if (
        ...
        if((tmp = CGI_Find_Parameter(input, "type"))) { // for media // 1:music, 2:video, 3:photo
            ...
            if(source_file_uc && source_path_uc) {
                /* We got format of ROOT_PATH = "/tmp/mnt/fslite"
        ...
        if ( (tmp=CGI_Find_Parameter(input, "source_file")) ) {
            ...
            sprintf(download_list, "%s/USB/%dwfm2", ROOT_PATH, irand);
...
int _download_urlenc(char *full_path, char *download_file, char *url_encoded_path, int compress, int multiple, long long offset)
{
    ...
    if (compress) {
        ...
        { 
            ...
            // printf("X-Sendfile2:/tmp/mnt/fslite/Photos/IMAG0017.jpg 0-893495\n");
            printf("Content-Type: application/force-download%c%c%c%c", 13, 10, 13, 10);
        }
