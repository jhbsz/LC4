...
int op_get_domain_ip_list(DOMAIN_IP_INFO *aDomain_ip_info)
{
    ...
    ezplib_get_attr_val("myqnapcloud_id_rule", 0, "device_name", qname, sizeof(qname), 0);
    if(qname[0]) {
        sprintf(_domain_ip_info.mycloudnas_dn, "%s.myqnapcloud.com", qname);
    } else {
        _domain_ip_info.mycloudnas_dn[0] = 0;
    }
...
static char *qfile_dirs[20] = {NULL};
static int fldcnt = 0;
int valid_root_folder(char *dirname) {
    int i;
    char *validp;
    char buf[30];
    int rulecnt = 0;
    if(fldcnt == 0) {
        /* Add default folders */
        validp = nvram_get("st_defpath_rule_num");
        if(validp && validp[0]) {
            rulecnt = atoi(validp);
        }
        for(i = 0;i < rulecnt; i++) {
            ezplib_get_attr_val("st_defpath_rule", i, "linkp", buf, sizeof(buf), 0);
            qfile_dirs[i] = strdup(buf);
        }
        fldcnt += rulecnt;
    }
    if(fldcnt != 0) {
        for(i = 0;i < fldcnt; i++) {
            if(!strcmp(dirname, qfile_dirs[i])) {
                return i;
            }
        }
    }
...
int get_root_share(int path_bool)
{
    ...
    char *qfile_dir[] = {
        ...
        "Documents",
        "Others"};*/
    /* Create an large enough temp array */
    struct dirent qfile_readdir[sizeof(qfile_dirs)/sizeof(qfile_dirs[0])];
    ...
    while((ptr = readdir(dir))!=NULL)
    {
        ...
        if((path_bool == 9) && strcmp(ptr->d_name, "SD") && strcmp(ptr->d_name, "USB"))
            continue;
        if((j = valid_root_folder(ptr->d_name)) >= 0) {
    ...
    for(i = 0; i < get_root_folder_num(); i++){
...
{
    char qsync_dir[MAX_FILE_PATH]={0};
    ...
    sprintf(qsync_dir, "%s/Qsync", ROOT_PATH);
    dir = opendir(path);
    while ((ptr = readdir(dir))!=NULL) {
        //do not include "." and ".."
        if (!strcmp(ptr->d_name, ".") || !strcmp(ptr->d_name, ".."))
        ...
        if(!strcmp(path, qsync_dir)){
            if(valid_root_folder(ptr->d_name) >= 0)
                continue;
        } 
        total++;
    }
