...
int load_one_part(struct cgi_state *cgi, FILE *f, int *len, char *boundary)
{
    ...
    const char *upload_id = upload_id = get(cgi, "upload_id");;
    char tmp_path[2048] = "";
    FILE *file = NULL;
...
void load_variables(struct cgi_state *cgi)
{
    ...
    /* url is upgrade.cgi fo ios */
    if(strstr(cgi->url, "upgrade.cgi") && !cgi->content_type) {
        return;
    }
	else if (cgi->content_length > 0 && cgi->request_post) {
