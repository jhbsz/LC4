...
extern void do_file(char *path, INPUT *input, ...);
...
extern void do_ej(char *path, INPUT *input, ...);
extern void do_ej_without_cgi(char *path, INPUT *input, ...);
struct ej_handler
{
    char *pattern;
    int (*output) (int eid, INPUT *input, int argc, char_t ** argv);
};
