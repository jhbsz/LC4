        ...
        int64_t *map)
{
    ...
    char *apn, *pin, *mode, *username, *passwd, *demand, *idletime, 
         ...
         *location, *isp, *echo_failure, *auth, *turbolink ,*failover;
    ...
    if(nmodel == 0 && nbrand == 13 && !strcmp(location,"0") && (!strcmp(isp,"2") || !strcmp(isp,"3")) ){
    
    	snprintf(tmp, sizeof(tmp), "wan%d_failover", idx);
    	failover = websGetVar(wp, tmp, "");

    	ezplib_replace_attr("als_failover",0,"enable",failover);   
	
	if(!strcmp(isp,"3"))
    	    ezplib_replace_attr("als_failover",0,"type","V");   
	if(!strcmp(isp,"2"))
    	    ezplib_replace_attr("als_failover",0,"type","T");   

        ezplib_replace_rule("netstat_rule", 0, "6");
    }
        ezplib_replace_rule("netstat_rule", 0, "3");
    len = 
...
ej_wan_barry_show_isp(int eid, webs_t wp, int argc, char_t **argv)
{
    ...
    websWrite(wp, "<select name='wan%d_barry_location' id='wan%d_barry_location' "
                        "value='%d' onchange='sel_barry_location(%d, %d);sel_barry_isp(%d)'>\n",
                        nth, nth, nlocation, nth, type);
