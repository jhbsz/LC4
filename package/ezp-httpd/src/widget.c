...
widget_show_statusinfo(webs_t wp)
{
    ...
    websWrite(wp, "<div widgetInfo>");
    websWrite(wp, "<a>Battery : </a> <a id='battery'></a>");
    websWrite(wp, "</div>");
#endif
    return;
}
...
#if defined(PLATFORM_ALS)
    websWrite(wp, "<tr>\n");
    websWrite(wp, "<td>%s</td>\n", lang_translate("Signal"));
    websWrite(wp, "<td><div id='wwan_signal_dbm' style='font-size:10px' width=20>"
                            "&nbsp;</div>\n");
    websWrite(wp, "<input type='button' value='Search' id='rssi_search' /><script type='text/javascript'>"
                  "$('#rssi_search').click(function(){"
                  "$(this).attr('disabled',true);"
                  "$.ajax({type: 'POST',url: 'status.cgi',data: {action:'rssi'},"
                  "success: function(i){if(i){$('#wwan_signal_dbm').html(i+' dBm');alert('Get : ' + i + ' dBm');}},"
                  "error: function(xhr, ajaxOptions, thrownError){alert('Get fail,Please try again!!'); }"
                  "});search_cd(\"30\");});"
                  "function search_cd(i){i--;if(i > 0){$('#rssi_search').val(i);setTimeout(\"search_cd(\" +i +\")\",1000);}else{$('#rssi_search').val('Search');$('#rssi_search').attr('disabled',false);}return;}"
                            "</script></td>\n");
    websWrite(wp, "</tr>\n");
