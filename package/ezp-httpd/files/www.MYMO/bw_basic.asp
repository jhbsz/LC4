...
function _local_sbm_rule_check(T) {
    ...
    rule = local_generate_rule('sbm_table');
    for (var i = 0; i < <%nv_get("wan_num");%>; i++) {
        var total_dl = parseInt(E('wan'+i+'_bw_dl').value);
        var total_ul = parseInt(E('wan'+i+'_bw_ul').value);
        if (rule[enum_sbm._EXTIF] == 'wan'+i) {
            if (rule[enum_sbm._BW_ALLOC] == 'ratio') {
...
function _local_create_sbm_input_field(T, nth) {
    ...
    if (rule[enum_sbm._BW_ALLOC] == 'ratio') {
        ...
        s += "<div class='setting'>"
            ...
            + "<input id='sbm_table_bw_download' name='sbm_table_bw_download' size='7' maxlength='6' "
            +       "onblur='sbm_update_available_bw()' /> Kbps"
            +"</div>";
        s += "<div class='setting'>"
            + "<div class='ilabel'>"+'<%lang("Upload");%>' + "</div>"
            + "<input id='sbm_table_bw_upload' name='sbm_table_bw_upload' size='7' maxlength='6' "
            +       "onblur='sbm_update_available_bw()' /> Kbps"
            +"</div>";
    }
    s += "<div class='setting'>"
    ...
    if (rule[enum_sbm._EXCESS] == '1') {
        if (rule[enum_sbm._BW_ALLOC] == 'ratio') {
            ...
            s += "<div class='setting'>"
                ...
                + "<input id='sbm_table_excess_download' name='sbm_table_excess_download' size='7' maxlength='6' "
                +       "onblur='sbm_update_available_bw()' /> Kbps"
                +"</div>";
            s += "<div class='setting'>"
                + "<div class='ilabel'>"+'<%lang("Use Maximal Upload");%>' + "</div>"
                + "<input id='sbm_table_excess_upload' name='sbm_table_excess_upload' size='7' maxlength='6' "
                +       "onblur='sbm_update_available_bw()' /> Kbps"
                +"</div>";
        }
    }
