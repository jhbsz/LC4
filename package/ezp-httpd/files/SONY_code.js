function change_language(F){
    var LangForm = document.getElementsByName("LangForm")[0];
...
function do_header_pre() {
	...
	*/
	...
	W("<option value='JP' "+'<% nvg_match("lang", "JP", "selected"); %>'+">日本語</option>");
	W("<option value='_FR' "+'<% nvg_match("lang", "_FR", "selected"); %>'+">Français</option>");
	W("<option value='DE' "+'<% nvg_match("lang", "DE", "selected"); %>'+">Deutsch</option>");
	W("<option value='IT' "+'<% nvg_match("lang", "IT", "selected"); %>'+">Italiano</option>");
	W("<option value='NL' "+'<% nvg_match("lang", "NL", "selected"); %>'+">Nederlands</option>");
	W("<option value='ES' "+'<% nvg_match("lang", "ES", "selected"); %>'+">Español</option>");
	W("<option value='KO' "+'<% nvg_match("lang", "KO", "selected"); %>'+">한국어</option>");
	W("<option value='C_T' "+'<% nvg_match("lang", "C_T", "selected"); %>'+">繁體中文</option>");
	W("<option value='C_S' "+'<% nvg_match("lang", "C_S", "selected"); %>'+">简体中文</option>");
	W("<option value='AR' "+'<% nvg_match("lang", "AR", "selected"); %>'+">العربية</option>");
	W("<option value='TK' "+'<% nvg_match("lang", "TK", "selected"); %>'+">Türkiye</option>");
...
function create_waiting_window_msg(msg) {
    ...
    lang = '<% nv_get("lang"); %>';
    if( lang == "AR") {
        W("<span id='waiting-window-msg' style='font-size:20px;text-align:right;' dir='RTL'>" + msg + "</span>");
    } else {
...
function create_input_window(T, U, fixed, x) {
    ...
    W("<input type='button' name='Add' value='" + '<%lang("OK");%>' + "' onClick='tbl_add_rule(\""+T+"\", "+U+")'/>");
...
function create_return_window(reload_page) {
    ...
    lang = '<% nv_get("lang"); %>';
    if(lang == "AR") {
        W("<div id='return-window-message' style='font-size:14px;text-align:right;' dir='RTL'></div>");
    } else {
        W("<div id='return-window-message' style='font-size:14px;'></div>");
    }
    W("<br/><br/><br/><br/>");
    if (typeof(reload_page)=="undefined") {
        W("<input type='button' id='return-OK' name='OK' value='" + '<%lang("OK");%>' + "' onClick='window.location.reload()'/>");
    }
    else {
        W("<input type='button' id='return-OK' name='OK' value='" + '<%lang("OK");%>' + "' onClick='window.location.href=\"" + reload_page + "\";'/>");
    }
    W("</div>");
    do_body_post();
    W("</div>");
...
function create_upgrade_window() {
    ...
    lang = '<% nv_get("lang"); %>';
    if( lang == "AR") {
        W('<font dir="RTL"><%lang("Updating…");%><br>');
        W('<%lang("Do not press the power button or the reset button.");%><br></font>');
    } else {
        W('<%lang("Updating…");%><br>');
        W('<%lang("Do not press the power button or the reset button.");%><br>');
    }
    do_body_post();
    W("</div>");
    W("</div>");
