	...
	for (i = 0; i < ret.length; i++) {
		...
		for (j = 0; j < <%nv_get("wan_num");%>; j++) {
			...
			if (pair[0] == 'Battery') {
				switch(pair[1]){
				    case '1':
				        $("#battery").html("Adaptor Mode");
				    break;
				    case '2':
				        $("#battery").html("Battery Mode, 80%");
				    break;
				    default :
				        $("#battery").html("Battery Low, 20%");
				    break;
				}
			}	
			...
			if (pair[0] == 'wan'+j+'_ipaddr') {
				...
				if($('#rssi_search') && pair[1] != '0.0.0.0' && $('#rssi_search').val() == 'Search')
				{
					$('#rssi_search').attr('disabled',false);
				}
			}
...
<%prod_show_js_start("wwan", "0", "7");%>
		...
		if (pair[0] == 'wwan_on' && pair[1] == '0') {
			...
			if (E('wwan_signal_als') != undefined) {
				E('wwan_signal_als').innerHTML = '&nbsp;';
			}
		}
		...
		if (wwan_on == '1' && pair[0] == 'Signal') {
			if (E('wwan_signal_dbm') != undefined) {
				E('wwan_signal_dbm').innerHTML = pair[1] + "dBm";
			}
		}
