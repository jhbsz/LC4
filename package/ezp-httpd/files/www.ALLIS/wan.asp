...
<%prod_show_js("end", "wisp");%>
    ...
    function failover(){
	if( $("#wan0_wwan_model").val() == 0 && $("#wan0_wwan_brand").val() == 13 && $("#wan0_wwan_location").val() == 0 && ($("#wan0_wwan_isp").val() == 2 || $("#wan0_wwan_isp").val() == 3 ) && $("#failover")){
	    $("#failover").attr("style","");
	}else
	{
	    $("#failover").attr("style","display:none");
	}
    }
	...
	failover();    
        var origin_locat = '<%nvg_attr_get("", "wan_wwan_rule", "$0","location");%>';                                             
        var origin_isp = '<%nvg_attr_get("", "wan_wwan_rule", "$0","isp");%>';                                             
        var v1 = parseInt(E('wan'+nth+'_wwan_location').value);
        var v2 = parseInt(E('wan'+nth+'_wwan_isp').value);
        if ( origin_locat ==  v1 && origin_isp == v2 )return;
        var isp = isps[v1][v2]
