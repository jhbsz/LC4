...
/* Enable RX and TX flow control on all ports */
...
int
athrs16_ioctl(struct net_device *dev, void *args, int cmd)
{
    ...
    ethcfg = (struct eth_cfg_params *)ifr->ifr_data;
     ...
     if(cmd  == RD_PHY) {
        if(ethcfg->portnum != 0xf)
            ...
            ethcfg->val = athrs16_reg_read(ethcfg->phy_reg);
    }
    else if(cmd  == WR_PHY) {
        if(ethcfg->portnum != 0xf)
            phy_reg_write(0,ethcfg->portnum,ethcfg->phy_reg,ethcfg->val);
        else
            athrs16_reg_write(ethcfg->phy_reg,ethcfg->val);
    }
         ...
         if(ethcfg->phy_reg < ATHR_PHY_MAX) {
	       ...
	       printk("Forcing 10Mbps %s on port:%d \n",
               ...
               athrs16_force_10M(ethcfg->phy_reg,ethcfg->duplex);
            }
            else if(ethcfg->val == 100) {
	       printk("Forcing 100Mbps %s on port:%d \n",
                         dup_str[ethcfg->duplex],(ethcfg->phy_reg));
               athrs16_force_100M(ethcfg->phy_reg,ethcfg->duplex);
            }
