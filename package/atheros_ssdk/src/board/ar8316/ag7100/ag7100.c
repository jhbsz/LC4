...
#ifdef CONFIG_AR9100
        ag7100_reg_wr(mac, AG7100_MAC_FIFO_CFG_3, 0xfff0078);
...
#if defined(CONFIG_ATHRS26_PHY) || defined(CONFIG_ATHRS16_PHY)  && defined(HEADER_EN)
...
#elif defined CONFIG_ATHRS16_PHY
        memmove(&skb->data[0], &skb->data[2], 12);
        skb->data[13] = 0x20; /* broadcast = 0; from_cpu = 0; version = 2; port_num = 0 */
        skb->data[12] = 0x80; /* version = 0b10; priority = 0; type = 0 (normal) */
#endif
    }
...
process_pkts:
    ...
    */
    ...
    while(quota)
    {
        ...
        if(mac->mac_unit == 0)
        {
            type = (skb->data[head_offset]) & 0xf;

            if (type == NORMAL_PACKET)
            {
#if defined (CONFIG_ATHRS26_PHY) && defined(CONFIG_VLAN_8021Q) || defined(CONFIG_VLAN_8021Q_MODULE)            	
                /*cpu egress tagged*/
                if (is_cpu_egress_tagged())
                {
                    if ((skb->data[12 + HEADER_LEN] != 0x81) || (skb->data[13 + HEADER_LEN] != 0x00))
                    {
                        int def_vid = athrs26_defvid_get(skb->data[0] & 0xf);
                        skb_push(skb, VID_LEN); /* vid lenghth - header length */

                        memmove(&skb->data[0], &skb->data[VID_LEN+2], 12); /*remove header and add vlan tag*/
...
#endif
                {
        	        ...
        	        memmove(&skb->data[2], &skb->data[0], head_offset);
#endif
                    skb_pull(skb, HEADER_LEN); /* remove attansic header */
                }
...
ag7100_get_default_macaddr(ag7100_mac_t *mac, u8 *mac_addr)
{
    ...
    */
    ...
    else
    {
     ...
     mac_addr[5] = jiffies & 0xff;
}


#define OLDDEBUG_INFO 1
#ifdef OLDDEBUG_INFO
#define AG7100CGMIIREG 1
#define AG7100CSMIIREG 2
#define AG7100CSMAC    3
#define AG7100CGDEBUG  4
#define AG7100CSDEBUG  5
#define AG7100CGCPUREG 6
#define AG7100CSCPUREG 7
...
ag7100_do_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
    	...
    	switch (args[0]) {
		    case AG7100CGMIIREG:
		    {
		        ...
		        regVal = (unsigned long)ag7100_mii_read(0, args[1], args[2]);
		        if (copy_to_user((void __user *)args[3], &regVal, sizeof(unsigned long)))
		        	return EFAULT;
		        return 0;
		    }
		    case AG7100CSMIIREG:
		    {   
		        ag7100_mii_write(0, args[1], (uint8_t)args[2], (uint16_t)args[3]);
		        return 0;
		    }
		    case ATHRCGREG: {
		        uint32_t reg_val=0;
...
#elif CONFIG_ATHRS16_PHY
		        reg_val = (uint32_t)athrs16_mdio_read(args[1]);
#endif
		        if (copy_to_user((void __user *)args[2], &reg_val, sizeof(uint32_t)))
		            return EFAULT;
		        return 0;
		    }
		    case ATHRCSREG: {
#ifdef CONFIG_ATHRS26_PHY		        
		        athrs26_reg_write (args[1], (uint32_t)args[2]);
#elif CONFIG_ATHRS16_PHY
		        athrs16_mdio_write (args[1], (uint32_t)args[2]);
#endif              
		        return 0;
		    }
			case AG7100CSMAC:
			{
        		memset((void *)&mac->mac_net_stats, 0, sizeof(mac->mac_net_stats));
        		ag7100_set_mac_from_link(mac, (ag7100_phy_speed_t)args[1], args[2]);
        		netif_carrier_on(dev);
        		netif_start_queue(dev);
        		return 0;
			}
            case AG7100CGCPUREG : {
                uint32_t reg_val = ar7100_reg_rd(args[1]);
                printk("read cpu register: addr:0x%lx  val:0x%x\n", args[1], reg_val);
                if (copy_to_user((void __user *)args[2], &reg_val, sizeof(uint32_t)))
		            return EFAULT;
		        return 0;
            }
            case AG7100CSCPUREG : {
                printk("write cpu register: addr:0x%lx  val:0x%lx\n", args[1], args[2]);
                ar7100_reg_wr(args[1], (uint32_t)args[2]);
                return 0;
            }            
	   	}
