...
{
    a_uint32_t busy = 1, i = 0x1000, entry;
    sw_error_t rv;
...
static sw_error_t
_isis_ip6_base_addr_set(a_uint32_t dev_id, fal_ip6_addr_t * addr)
{
    sw_error_t rv;
    a_uint32_t i, baseaddr, reg[3];

    reg[0] = addr->ul[0];
    reg[1] = addr->ul[1] & 0xffffff00;
    reg[1] |= ((addr->ul[2] >> 24) & 0xff);
    reg[2] = (addr->ul[2] & 0xffffff) << 8;
    reg[2] |= ((addr->ul[3] >> 24) & 0xff);

    for (i = 0; i < 3; i++) {
        baseaddr = ISIS_IP6_BASE_ADDR + (i << 2);
        HSL_REG_ENTRY_GEN_SET(rv, dev_id, baseaddr, sizeof (a_uint32_t),
...
static sw_error_t
_isis_ip6_base_addr_get(a_uint32_t dev_id, fal_ip6_addr_t * addr)
{
    sw_error_t rv;
    a_uint32_t i, baseaddr, reg[3];

    aos_mem_zero(addr, sizeof (fal_ip6_addr_t));
    for (i = 0; i < 3; i++) {
        baseaddr = ISIS_IP6_BASE_ADDR + (i << 2);
        HSL_REG_ENTRY_GEN_GET(rv, dev_id, baseaddr, sizeof (a_uint32_t),
                              (a_uint8_t *) (&(reg[i])), sizeof (a_uint32_t));
        SW_RTN_ON_ERROR(rv);
    }

    addr->ul[0] = reg[0];
    addr->ul[1] = reg[1] & 0xffffff00;
    addr->ul[2] = (reg[1] & 0xff) << 24;
    addr->ul[2] |= ((reg[2] >> 8) & 0xffffff);
    addr->ul[3] = (reg[2] & 0xff) << 24;

    return SW_OK;
}

/* in hardware host entry only interface id low 24bits and subnet id
   low 8 bits is valid */
static sw_error_t
_isis_ip6_addr_sw_to_hw(a_uint32_t dev_id, fal_ip6_addr_t sw_addr,
                        a_uint32_t * hw_addr)
{
#if 0
    sw_error_t rv;
    ...
    rv = _isis_ip6_base_addr_get(dev_id, &addr);
    SW_RTN_ON_ERROR(rv);
    ...
    for (i = 0; i < 4; i++) {
        if ((msk[i] & sw_addr.ul[i]) != (msk[i] & (addr.ul[i]))) {
            return SW_BAD_PARAM;
        }
    }
#endif

    *hw_addr = (sw_addr.ul[3] & 0xffffff) | ((sw_addr.ul[1] & 0xff) << 24);
    return SW_OK;
}

static sw_error_t
_isis_ip6_addr_hw_to_sw(a_uint32_t dev_id, a_uint32_t hw_addr,
                        fal_ip6_addr_t * sw_addr)
{
    sw_error_t rv;
    a_uint32_t i;
    fal_ip6_addr_t addr;
    ...
    for (i = 0; i < 4; i++) {
        sw_addr->ul[i] = addr.ul[i];
    }

    sw_addr->ul[3] |= (hw_addr & 0xffffff);
    sw_addr->ul[1] |= ((hw_addr >> 24) & 0xff);
    return SW_OK;
}
                    ...
                    a_uint32_t reg[])
{
    ...
    if (FAL_IP_IP6_ADDR & entry->flags) {
        rv = _isis_ip6_addr_sw_to_hw(dev_id, entry->ip6_addr, &(reg[0]));
        SW_RTN_ON_ERROR(rv);

        SW_SET_REG_BY_FIELD(HOST_ENTRY3, IP_VER, 1, reg[3]);
    }
                    ...
                    fal_host_entry_t * entry)
{
    ...
    if (data) {
        rv = _isis_ip6_addr_hw_to_sw(dev_id, reg[0], &(entry->ip6_addr));
        SW_RTN_ON_ERROR(rv);
...
#define ISIS_WCMP_HASH_TBL_ADDR   0x0e00
...
/**
 * @brief Set IP6 base address one particular device.
 *   @details Comments:
 *     Hardware only store 96 bits address as base address.
 * @param[in] dev_id device id
...
isis_ip6_base_addr_set(a_uint32_t dev_id, fal_ip6_addr_t * addr)
{
    ...
    HSL_API_LOCK;
    rv = _isis_ip6_base_addr_set(dev_id, addr);
    HSL_API_UNLOCK;
    return rv;
}

/**
 * @brief Get IP6 base address one particular device.
 * @param[in] dev_id device id
...
isis_ip_init(a_uint32_t dev_id)
{
...
#ifndef HSL_STANDALONG
    {
        ...
        p_api->ip6_base_addr_set = isis_ip6_base_addr_set;
        p_api->ip6_base_addr_get = isis_ip6_base_addr_get;
    }
#endif
