                       ...
                       a_uint32_t cnt_id, a_bool_t enable)
{
    ...
    SW_SET_REG_BY_FIELD(HOST_ENTRY4, ENTRY_FUNC, ISIS_NAT_ENTRY_SEARCH, reg[4]);
...
_isis_napt_get(a_uint32_t dev_id, a_uint32_t get_mode, fal_napt_entry_t * entry)
{
    ...
    #if 1
 	entry->entry_id = ( reg[4]>>8 )& 0x3ff;
	int found  = (reg[4]>>7 )& 0x1;
    int addr = 0x2000c | (entry->entry_id <<4);
    int age = 0;

    HSL_REG_ENTRY_GEN_GET(rv, dev_id, addr, sizeof (a_uint32_t),
                              (a_uint8_t *) (&age), sizeof (a_uint32_t));
    age = (age >>12)&0xf;
    found = found & age;
    #endif
                        ...
                        a_uint32_t cnt_id, a_bool_t enable)
{
    ...
    SW_SET_REG_BY_FIELD(HOST_ENTRY4, ENTRY_FUNC, ISIS_NAT_ENTRY_NEXT, reg[4]);
