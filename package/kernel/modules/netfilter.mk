...
define KernelPackage/ipt-webstr
  SUBMENU:=$(NF_MENU)
  TITLE:=Module for web string
  KCONFIG:=$(KCONFIG_IPT_WEBSTR)
  FILES:=$(foreach mod,$(IPT_WEBSTR-m),$(LINUX_DIR)/net/$(mod).$(LINUX_KMOD_SUFFIX))
  AUTOLOAD:=$(call AutoLoad,45,$(notdir $(IPT_WEBSTR-m)))
  DEPENDS:= kmod-ipt-core
endef

define KernelPackage/ipt-webstr/description
 Netfilter (IPv4) module for web string
endef

$(eval $(call KernelPackage,ipt-webstr))
