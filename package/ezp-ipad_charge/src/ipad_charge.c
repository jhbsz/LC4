...
#define PRODUCT_IPAD2_WIFI  0x129f
#define PRODUCT_IPAD2_GSM  0x12a2
#define PRODUCT_IPAD2_CDMA  0x12a3
#define PRODUCT_IPAD2_R2  0x12a9
#define PRODUCT_NEW_IPAD_WIFI	0x12a4
#define PRODUCT_NEW_IPAD_CDMA	0x12a5
#define PRODUCT_NEW_IPAD_GLOBAL	0x12a6
		...
		while ((dev = devs[i++]) != NULL) {
			...
			if (desc.idVendor == VENDOR_APPLE && \
               (desc.idProduct == PRODUCT_IPAD || \
                desc.idProduct == PRODUCT_IPAD2_WIFI || \
                desc.idProduct == PRODUCT_IPAD2_GSM || \
                desc.idProduct == PRODUCT_IPAD2_CDMA || \
                desc.idProduct == PRODUCT_IPAD2_R2 || \
                desc.idProduct == PRODUCT_NEW_IPAD_WIFI || \
                desc.idProduct == PRODUCT_NEW_IPAD_CDMA || \
                desc.idProduct == PRODUCT_NEW_IPAD_GLOBAL)) {
				if (set_charging_mode(dev, enable) < 0)
					fprintf(stderr, "ipad_charge: error setting charge mode\n");
				else
					count++;
