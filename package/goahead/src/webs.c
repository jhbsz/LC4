...
int websOpenServer(int port, int retries)
{
	...
	/*Start:Add for "HTTP port in LAN" by Steve, 2009-02-09*/
	#if 1
	char TempBuf[32]={0};
 ...
 */
unsigned int total_file_len=0; //Steve patched 2010/05/26
...
//Steve patched 2010/05/26
	if(len_aa>7800000)
       {
       	printf("%s => dont_write_to_ram==>len_aa > 7800000\n", __FUNCTION__);
	len_aa=0;
        dont_write_to_ram=0;
	websRedirect(wp, "/local/advance/redirect2uploadpage.asp");
       }
        else
       {
	len_aa=0;
 ...
 */
         ...
         if (wp->clen > 0)
         {
			   ...
			   websSetVar(wp, T("CONTENT_LENGTH"), value);
         	           //Steve patched 2010/05/26
         	   printf("\n%s => wp->clen=%d\n", __FUNCTION__, wp->clen);
                           if((wp->clen)>7800000)
			   {		   
			   		printf("%s => wp->clen > 7800000\n", __FUNCTION__);
			   		printf("%s => dont_write_to_ram=1 \n",__FUNCTION__);
			   dont_write_to_ram=1;
			   }
			   else{
			   		printf("%s => dont_write_to_ram=0 \n",__FUNCTION__);
			   		dont_write_to_ram=0;//Steve
               }
         }
         else
         {
...
void websSetEnv(webs_t wp)
{
	...
	websSetVar(wp, T("SERVER_ADDR"), wp->ifaddr); //Steve pacthed
 ...
 */
	...
	wp->password = NULL;
	wp->userName = NULL;
	wp->has_firmware_upload_clean = 0; //Steve pacted 2010/05/26
#ifdef DIGEST_ACCESS_SUPPORT
	wp->realm = NULL;
	wp->nonce = NULL;
	wp->digest = NULL;
