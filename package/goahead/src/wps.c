...
// Tommy ,WPS stop, SSID = NBG419N_<half of Mac Address>
	...
	if(getAPMac("ra0", mac) != -1)
		...
		sprintf(SSID, "RalinkInitAP_unknown");
#else
// Tommy, ZyXEL bug No. 29: After OOB, don't change SSID, Security ...
	sprintf(SSID, "ZyXEL");		
#endif		
	// Need to Implement with AXIMCOM ???, ok
	//nvram_bufset(RT2860_NVRAM, "WscConfigured", "0");
...
static void WPSSTAPINStartReg(char *ssid, char *pin)
{
	...
	doSystem("iwpriv ra0 wsc_auto_conn 1");
...
// NBG4615: Don't change WLAN/WPS LED state, because WLAN/WPS LED make together now.
#if 0
	LedReset();
#endif
...
// Tommy, for PIN Code generate, 2008/12/24 01:18下午
...
static int getWISPConnectionStatus(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	system("iwconfig ra0 | grep 'Access Point:' | sed 's/^.*Access Point: //g' > /tmp/wisp_essid_mac");
...
static int getWISPapcli0ConnectESSID(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	system("iwconfig apcli0 | grep 'ESSID' | sed 's/^.*ESSID:\"//g' | sed 's/\".*$//g' > /tmp/wisp_essid_name");
	...
	if(!strcmp(buf, "" )){
		...
		ezplib_get_attr_val("wl0_apcli_rule", 0, "ssid", TempBuf, 128, EZPLIB_USE_CLI);
