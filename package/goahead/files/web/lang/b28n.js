...
//}
function showWebMessage(flag, str)
{
	var item = parent.getElement("message_str");
	if (flag==1)
		item.style.color="#ff3b06";
	else
	{	
		item.style.color="#3d8900";
		if (str=="Ready")
			str=_("main Ready");
		if (str=="Configuration updated successfully.")
			str=_("main ConfigurationSuccess");
			
		//----- aron add  2010.03.12
		if (str=="Firmware download in progress. Please wait . . .")
		        str=_("main firmware download in progress");
		if (str=="Firmware upgrade is proceeding. Please wait and do NOT power off your device.")
		        str=_("main firmware upgrade is proceeding");
		//-----------------------------
	}
	if (flag==2)
		str = _("main ConfigurationProceeding");
  item.innerHTML = _(str);
}
