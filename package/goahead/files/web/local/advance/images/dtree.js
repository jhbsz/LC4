...
function dTree(objName) {
	...
	this.icon = {
		...
		root				: 'images/space.gif',

		folder			: 'images/space.gif',

		folderOpen	: 'images/space.gif',

		node				: 'images/space.gif',
...
dTree.prototype.node = function(node, nodeId) {
	...
	if (node.url) {
		...
		if (this.config.useSelection && ((node._hc && this.config.folderLinks) || !node._hc))
			...
			str += ' onclick="javascript: if ('+ node.pid +') onChangeHeadMenu(' + node.id + ', 1, \'' + node.title + '\'); ' + this.obj + '.s(' + nodeId + ');"';
...
function onChangeHeadMenu(index1, index2, pathTitle_str)
{
	document.getElementById("pathTitle").innerHTML = pathTitle_str;
	finish=false;
	i=0;
	while(!finish)
	{
	 i++;
	 if (document.getElementById("menu_"+prev_menu_id+"_"+i)==null)
	 {
		finish=true;
	 }
	 else
	 {
		show_div(false, "menu_"+prev_menu_id+"_"+i);
		show_div(false, "menu_"+prev_menu_id+"_"+i+"_hightline");
	 }
	}
	finish=false;
	i=0;
	...
	{
	 i++;
	 if (document.getElementById("menu_"+index1+"_"+i)==null)
	 {
		finish=true;
	 }
	 else
	 {
		if (i==index2)
		{
			show_div(false, "menu_"+index1+"_"+i);
			show_div(true, "menu_"+index1+"_"+i+"_hightline");
		}
		else
		{
			show_div(true, "menu_"+index1+"_"+i);
			show_div(false, "menu_"+index1+"_"+i+"_hightline");
		}
	 }
	}
	prev_menu_id=index1;
	prev_submenu_index=index2;	
}
