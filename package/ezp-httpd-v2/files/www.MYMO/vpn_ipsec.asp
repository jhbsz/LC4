...
function local_generate_rule(T) {
    ...
    if (rule[enum_ipsec._MODE] == 'rw_server') {
	    ...
	    r[enum_ipsec._REMOTE_GATEWAY] = rule[enum_ipsec._REMOTE_GATEWAY];
	    r[enum_ipsec._REMOTE_INIPADDR] = rule[enum_ipsec._REMOTE_INIPADDR];
...
function _local_create_input_field(T, nth) {
    ...
    if ((rule[enum_ipsec._MODE] == 'net2net') ||
        (rule[enum_ipsec._MODE] == 'rw_server' && rule[enum_ipsec._L2TP] == '0')) {
        s += "<div class='setting'>"
            + "<div class='ilabel'>"+'<%lang("Local Internal IP Address");%>' + "</div>"
    ...
    if (rule[enum_ipsec._IKE_KEYMODE] == 'psk') {
        s += "<div class='setting'>"
            ...
            + "<input id='psk' name='psk' size='25' maxlength='40' />"
            +"</div>";
    }
