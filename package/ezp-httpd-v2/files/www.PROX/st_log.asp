	  ...
	  <script type="text/javascript">do_page_path('<%lang("Admin");%>', '<%lang("Log");%>')</script>
               <div id="contentsInfo">
                  ...
                  <div class="label"><% lang("Syslog Server"); %></div>
                    <div class='radiobutton'><input type="radio" value="1" name="remote_enable" onclick="init()"
                      <%nvg_attr_match("remote_enable","log_rule","$0","remote_enable","1","checked");%>>
                      <%lang("Enable");%>
                    </input></div>
                    <div class='radiobutton'><input type="radio" value="0" name="remote_enable" onclick="init()"
                  ...
                  <div class="label"><%lang("Remote IP Address");%></div>
                    <input type="text" maxLength="64" size="16" name="remote_ip_ipaddr"
                      value='<%nv_attr_get("log_rule", "$0", "remote_ip");%>' 
                      onblur='valid_reserved_word(this, "Remote IP Address")' />
                    </input>
                  </div>
                  <div class='setting'>
