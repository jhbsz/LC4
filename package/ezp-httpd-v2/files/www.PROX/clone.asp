  ...
  <script type="text/javascript">
    ...
    function init() {
        ...
        for (var i = 0; i < <%nv_get("wan_num");%>; i++) {
            ...
            sel_change(enable, 1, F, clone_wan_objects[i]);
        }
        for (var i = 0; i < <%nv_get("lan_num");%>; i++) {
            var enable = (document.getElementsByName("lan"+i+"_hwaddr_clone_enable")[0].checked == true) ? 1 : 0;
            sel_change(enable, 1, F, clone_lan_objects[i]);
        }
        <%widget_start();%>
    }
  </script>
