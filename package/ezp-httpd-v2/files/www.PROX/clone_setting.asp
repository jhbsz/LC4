...
[
  ...
  <fieldset>
    ...
    <%increase_wan("$2", "1");%></legend> <input type="hidden" name="$1$2_clone_http_client">

      <div class="setting">
        <div class="label">
          <% lang("Clone WAN MAC"); %>
        </div>
        <div class='radiobutton'><input type="radio" value="1" name= "$1$2_hwaddr_clone_enable" onclick= "init()" 
        <%nvg_attr_match("$1$2_hwaddr_clone_enable","$1_hwaddr_clone_rule","$2","enable","1","checked");%>><%lang("Enable");%></div>
        <div class='radiobutton'><input type="radio" value="0" name= "$1$2_hwaddr_clone_enable" onclick= "init()" 
