...
var default_rule = ['','1','','','','','','','','','','wan0','default',''];
...
function _local_create_input_field(T, nth) {
    ...
    s += "<div class='setting'>"
        ...
        + "<input type='hidden' id='rt_table_old_seq' value='"+nth+"' />"
        + "<input id='rt_table_seq' size='4' maxlength='4' value='"+nth+"' />"
        +"</div>";
    ...
    s += "<div class='setting'>"
        ...
        + "<input id='enable' name='enable' type='checkbox' value='0' />"
        +"</div>";
    ...
    if (rule[enum_lb_routing._PROTOCOL] == '') {
        s += "<div class='setting'>"
            ...
            +           "onblur='valid_range(this, 1, 65535, \"Service Port Range End\")' disabled />"
            +"</div>";
    } else {
        s += "<div class='setting'>"
            + "<div class='ilabel'>"+'<%lang("Service Port Range");%>' + "</div>"
    ...
    for (i = 1; i <= <%nv_get("lan_num");%>; i++) {
        s += "<option value='lan" + (i-1) + "' >LAN" + i + "</option>";
    }
    s +=  "</select>"
        +"</div>";
    ...
    if (rule[enum_lb_routing._ROUTING_TYPE] == 'default') {
        s += "<div class='setting'>"
            ...
            + "<input id='gateway_ipaddr' name='gateway_ipaddr' size='17' "
            +       "maxlength='15' disabled='true' "
            +       "onblur='valid_ipaddr(this, \"Gateway IP Address\")' />"
            +"</div>";
    } else {
        s += "<div class='setting'>"
            + "<div class='ilabel'>"+'<%lang("Gateway IP Address");%>' + "</div>"
            + "<input id='gateway_ipaddr' name='gateway_ipaddr' size='17' maxlength='15' "
            +       "onblur='valid_ipaddr(this, \"Gateway IP Address\")' />"
            +"</div>";
    }
